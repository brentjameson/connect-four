      ...
      if(e.target === entryPointDiv1) {
        ...
        for(let sq of column1) {
          ...
          if(sq1[0].hasChildNodes()){
            ...
            const newDiv = document.createElement("div")
            //next 3 lines = trying to link the number of the piece class to the number of the gameboard class
            let squareOneOfZero = JSON.stringify(sq1[0].classList)
            console.log(squareOneOfZero)
            newDiv.classList.add(squareOneOfZero)
            if (counter % 2 === 0){
